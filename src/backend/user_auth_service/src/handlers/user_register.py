...
def user_register_handler(data: dict[str, Any],
                          db=None) -> int | SpecialException:
    """
    ...
    try:
        ...
        log.debug(f"""Отвалидировал данные пользователя:
                  \n{user.print_profile()}""")

        log.debug("Проверяю, существует ли уже пользователь в БД")
