    ...
    async def decorated_function(request: Request, *args, **kwargs):
        ...
        with next(get_db_users()) as db:
            ...
            if not auth_token or auth_token.expires_at < datetime.now():
                ...
                raise SpecialException("Недействительный токен авторизации")
...
def admin_required(f):
    ...
    async def decorated_function(request: Request, *args, **kwargs):
        ...
        if current_user.user_role != "Admin":
            log.warning(f"Пользователь {current_user.id} не имеет прав Админа")
...
def same_user_required(f):
    """
    Проверка на то, совершает ли запрос тот же пользователь,
    чей ID передаётся в запросе.
    """
    @wraps(f)
    async def decorated_function(request: Request,
        ...
        if current_user.id != user_id:
            log.warning(f"""
Доступ запрещен для {current_user.id} к данным пользователя {user_id}
""")
            raise SpecialException("Доступ запрещен: несовпадение ID")
