...
def user_login_handler(data: dict[str, Any]):
    ...
    try:
        ...
        if result.get("status") == "success":
            token, token_expiry = result["data"]["token"], result["data"]["token-expiry"]
            ...
            raise SpecialException(f"Неизвестная ошибка при получении ответа: {result}")
...
def user_register_handler(data: dict[str, Any]) -> int | SpecialException:
    ...
    try:
        ...
        if result['status'] == "success":
            log.debug(f"Возвращаю ID пользователя: {result["data"]["user_id"]}")
...
def put_user_info_handler(user_id: UUID, data: dict[str, Any]) -> dict | SpecialException:
    """
    Отправляет запрос к USER API об изменении информации об аккаунте в user_database.
        Параметры:
            data: Словарь в формате response.json с инфой:
