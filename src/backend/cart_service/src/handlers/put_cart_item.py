...
def put_cart_item_handler(data: dict[str, Any], user_id: UUID, db=None):
    ...
    try:
        ...
        log.debug(f"""
Обновляю товар {item_id} с количеством {quantity} для пользователя {user_id}
                """)
        ...
        result = {"price": 119.90}  # временно без ITEM API
        ...
        if cart_item:
            log.debug(f"""
            Товар {item_id} уже есть в корзине. Обновляю количество и цену
                      """)
            cart_item.quantity = quantity
            cart_item.price = sum_price
        else:
            log.debug(f"""
            Товар {item_id} отсутствует в корзине. Добавляю новый товар.
                      """)
        ...
        db.commit()
        log.debug(f"""
        Товар {item_id} успешно добавлен/обновлён для пользователя {user_id}.
        """)
    finally:
        if db is not None:
            db.close()
