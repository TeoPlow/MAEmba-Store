...
def recommend_seasons_items(user_id:int, user_history:list, top_n=10) -> list:
    ...
    all_recommendations = pd.read_csv("all_recommendations.csv") # Датафрейм с информацией об истории рекомендаций пользователям
    
    items = pd.read_csv("items.csv") # Таблица для получения категорий товаров
    ...
    if not user_history:  # Если у пользователя нет истории покупок
        ...
        sales_prod = pd.read_csv('sales_prod.csv') # История покупок на нашем сервисе
